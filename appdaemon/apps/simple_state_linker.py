...
class SimpleStateLinker(BaseApp):
    """
    ...
    async def _setup_app(self):
        ...
        try:
            ...
            self.error(f"Could not find Area Handler app ('{AREA_HANDLER_APP_NAME}'). "
                       f"This app cannot function.")
            return

        self.listen_event(self._run_group_processing, EVENT_AREAS_UPDATED)
        ...
        if self._area_handler.get_all_areas():
            ...
            self._run_group_processing("manual_trigger", {}, {})

    def _run_group_processing(self, event_name, data, kwargs):
    ...
    def _state_change_cb(self, entity: str, attribute: str, old: str, new: str, kwargs: dict):
        ...
        self.run_in(self._release_group_lock, self.grace_period, group_key=group_key)

    def _release_group_lock(self, kwargs):
